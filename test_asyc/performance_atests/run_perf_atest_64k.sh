#! /bin/bash
#***************************************************************************
echo "***QZ_TOOL run_perf_atest_64K.sh start"
...
function performance_test_with_calgary
{
    ...
    if [ $Service == "comp" ];then
        ...
        do
		      taskset -c 0	../atest -m 4 -l 500 -t $Thread -B 0 -D $Service -L $Level -i $Testfile -T $Huffman -C $Buffersz >> result 2>> result_stderr &
        done
        wait
    else
        for((num = 0; num < $Num; num++))
...
#Parameter: NumProcess NumDcInstance Num Thread Service Level Testfile Huffman Buffersz  
#Parameter: 1             2           1   6        -       1   calgary.512M static  64k        
./server_cpu_usage.pl 7 NGINX
./memory_monitor.pl
performance_test_with_calgary 1 2 1 6 comp 1 calgary.512M static 65536
...
./server_cpu_usage.pl 7 NGINX
./memory_monitor.pl
performance_test_with_calgary 1 2 1 6 decomp 1 calgary.512M static 65536
./read_server_cpu_usage_nginx.pl 7
./read_memory.sh
sleep 10
...
echo "***QZ_TOOL run_perf_atest_64K.sh end"
exit 0

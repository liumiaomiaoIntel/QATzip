...
usage() {
    ...
    echo "./cpu_usage -t<TIMER> -u -k -i -c -s -n <SERVER>"
    ...
    echo "-n Print total number of servers."
...
while getopts "t:ukicsan" OPTION
do
    case $OPTION in
        t)
            TIMER=$OPTARG
...
NUM_CORES=`expr $NUM_SERVERS - 1`
cat /proc/stat > /opt/cputemp
for f in `seq 0 $NUM_CORES`
  do
    CURRENT_CPU_INFO=(`cat /opt/cputemp | grep "cpu$f "`)   # Get the aggregated information
    ...
    USER_START=`expr $USER_START + ${CURRENT_CPU_INFO[1]}`                  # Store the user mode time at the start
    KERNEL_START=`expr $KERNEL_START + ${CURRENT_CPU_INFO[3]}`                 # Store the kernel mode time at the start
    IDLE_START=`expr $IDLE_START + ${CURRENT_CPU_INFO[4]}`                   # Store the idle time time at the start
done
rm -rf /opt/cputemp

sleep $TIMER

cat /proc/stat > /opt/cputemp          # copy to temp file as parsing different cores may take time
TOTAL_END=0                                       
for f in `seq 0 $NUM_CORES`
  do
    CURRENT_CPU_INFO=(`cat /opt/cputemp | grep "cpu$f "`)   # Get the aggregated information again
    ...
    USER_END=`expr $USER_END + ${CURRENT_CPU_INFO[1]}`
    KERNEL_END=`expr $KERNEL_END + ${CURRENT_CPU_INFO[3]}`
    IDLE_END=`expr $IDLE_END + ${CURRENT_CPU_INFO[4]}`
done
rm -rf /opt/cputemp
